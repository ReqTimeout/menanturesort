...
**Total URL ter-index:** 175 URL (148 artikel SEO + 12 pagination + 15 halaman utama)

---

## 🎯 Yang Harus Dikirim ke Google Search Console

**HANYA submit 1 URL ini** ke Google Search Console:
```
https://menantu-resort.com/sitemap-index.xml
```

**Jangan** submit `sitemap-0.xml` — itu file internal yang di-reference oleh `sitemap-index.xml`. Google akan otomatis crawl semua URL di dalamnya.

**Cara verifikasi sitemap live:**
```bash
curl -I https://menantu-resort.com/sitemap-index.xml
# Expected: HTTP/1.1 200 OK
```

**Cara verifikasi URL count:**
```bash
...
5. **Input field** "Add a new sitemap" → masukkan:
   ```
   sitemap-index.xml
   ```
6. Klik **Submit**
...
> **Note:** Google mungkin split jadi 2 sitemap (sitemap-0.xml + sitemap-1.xml) kalau URL > 50.000. Sekarang masih 175 URL, jadi 1 file cukup.

---

## 🔥 Step 3: Request Indexing — Batch Strategy (12-15 hari)
...
> **Tambahan:** Pagination URL `/artikel/page/2/` sampai `/artikel/page/13/` akan otomatis ter-index oleh Google via sitemap (tidak perlu manual request — itu secondary page, bukan target high-intent keyword).
...
**Q: Pagination URL perlu di-request indexing juga?**
A: Tidak wajib. Google akan otomatis crawl `/artikel/page/2/` sampai `/artikel/page/13/` via internal link dari `/artikel/`. Pagination URL adalah secondary, tidak target high-intent keyword — jadi prioritas rendah.
...
*Last updated: 2026-06-17 · Menantu Resort SEO Build · 175 URL sitemap (148 artikel + 12 pagination + 15 halaman utama) · Generated by Codex*
